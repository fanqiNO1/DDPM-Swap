...
def main(args):
    if args.load:
        # Load model
        pass
    unet = Unet(n_channels=args.n_channels, n_groups=args.n_groups).to(args.device)
    unet_optim = torch.optim.Adam(unet.parameters(), lr=args.unet_lr)
    ddpm = DenoiseDiffusion(eps_model=unet, n_steps=args.n_steps, device=args.device)
    ae = AE(n_blocks=args.ae_blocks).to(args.device)
    ae_optim = torch.optim.Adam(ae.parameters(), lr=args.ae_lr)
    arcface_ckpt = torch.load(args.arcface_path, map_location=torch.device("cpu"))
    ...
    for epoch in range(args.epochs):
        ...
        target_images_112 = F.interpolate(target_images, size=(112, 112), mode="bicubic")
        ...
        fake_latent = arcface(fake_images)
        ...
        # Print loss
        if epoch % args.loss_interval == 0:
            print(f"Epoch {epoch}: unet loss: {unet_loss.item():.4f}, ae loss: {ae_loss.item():.4f}")
        # Plot
...
if __name__ == "__main__":
    ...
    # For Unet
    parser.add_argument("--n_channels", type=int, default=16, help="number of channels")
    parser.add_argument("--n_groups", type=int, default=8, help="number of group norm")
    parser.add_argument("--unet_lr", type=float, default=2e-5, help="unet learning rate")
    # For DDPM
    ...
    # For AE
    parser.add_argument("--lambda_id", type=float, default=30.0, help="weight for id loss")
    parser.add_argument("--lambda_rec", type=float, default=10.0, help="weight for rec loss")
    parser.add_argument("--ae_lr", type=float, default=2e-5, help="ae learning rate")
    parser.add_argument("--ae_blocks", type=int, default=7, help="number of blocks")
    # For Arcface
    parser.add_argument("--arcface_path", type=str, default="", help="arcface path")
    # For train
    ...
    parser.add_argument("--save_interval", type=int, default=50000, help="save interval")
    parser.add_argument("--save_path", type=str, default="", help="save path")
    parser.add_argument("--loss_interval", type=int, default=100, help="loss interval")
    parser.add_argument("--loss_path", type=str, default="", help="loss path")
    parser.add_argument("--sample_interval", type=int, default=2000, help="sample interval")
    parser.add_argument("--sample_path", type=str, default="", help="sample path")
